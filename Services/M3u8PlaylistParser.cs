...
public static class M3u8PlaylistParser
{
    ...
    /// <summary>
    /// 选择 HLS 变体。targetQuality &gt; 0 时优先选分辨率最接近的变体（例如用户选了 480p 就不该下 1080p），
    /// 否则维持原来的「最高带宽」行为。
    /// </summary>
    public static M3u8Variant SelectBestVariant(IEnumerable<M3u8Variant> variants, int targetQuality = 0)
    {
        var list = variants as IList<M3u8Variant> ?? variants.ToList();
        if (targetQuality > 0)
        {
            var withHeight = list.Where(variant => variant.Height is > 0).ToList();
            if (withHeight.Count > 0)
            {
                return withHeight
                    .OrderBy(variant => Math.Abs(variant.Height!.Value - targetQuality))
